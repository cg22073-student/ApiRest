...
REQUEST_TIMEOUT = 2
RESOURCE_PATH = "tipoalmacenes"


class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Cliente REST - TipoAlmacen ")
        self.resize(900, 600)
        ...
        form_group = QGroupBox("Datos de TipoAlmacen")
        form_layout = QFormLayout()
        form_group.setLayout(form_layout)

        self.id_spin = QSpinBox()
        self.id_spin.setMinimum(0)
        self.id_spin.setMaximum(200)
        ...
        form_layout.addRow(QLabel("Observaciones:"), self.observaciones_edit)
        ...
        self.btn_crear = QPushButton("Nuevo Tipo Almacen")
        self.btn_actualizar = QPushButton("Actualizar Tipo Almacen")
        self.btn_eliminar = QPushButton("Eliminar Tipo Almacen")
    ...
    def listar_todos(self):
        url = f"{BASE_URL}/{RESOURCE_PATH}/all"
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                if not data:
                    self.append_output("No hay registros de TipoAlmacen.")
                    return
                self.append_output("----------------------------------------")
                for item in data:
                    ...
                    self.append_output(f"Observaciones: {item.get('obsevaciones')}")
                    self.append_output("----------------------------------------")
            else:
                self.mostrar_error(
                    "Error al listar.",
    ...
    def _buscar_por_id(self, mostrar_mensajes: bool):
        ...
        try:
            ...
            if resp.status_code == 200:
                item = resp.json()
                self.output.clear()
                self.append_output("=== DETALLE TIPO_ALMACEN ===")
                self.append_output(f"ID           : {item.get('id')}")
                self.append_output(f"Nombre       : {item.get('nombre')}")
                self.append_output(f"Activo       : {item.get('activo')}")
                self.append_output(f"Observaciones: {item.get('obsevaciones')}")
                self.nombre_edit.setText(item.get("nombre") or "")
                self.observaciones_edit.setText(item.get("obsevaciones") or "")
                self.activo_check.setChecked(bool(item.get("activo")))
            elif resp.status_code == 404:
                self.output.clear()
                self.append_output(f"No se encontró TipoAlmacen con id {id_val}.")
    ...
    def crear(self):
        ...
        payload = {
            "nombre": nombre,
            "activo": activo,
            "obsevaciones": observaciones or None
        }
